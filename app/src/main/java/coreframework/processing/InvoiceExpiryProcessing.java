...
public class InvoiceExpiryProcessing extends BackgroundProcessingAbstractFilter {

    private InvoiceExpiry request;
    ...
    public InvoiceExpiryProcessing(InvoiceExpiry request, CoreApplication application, boolean isPost) {
    ...
    public String captureURL() {
        ...
        this.request.setG_transType(TransType.INVOICE_EXPIRY_REQUUEST.name());
    ...
    public void processResponse(Message msg) {
        if (msg.arg1 == ServerConnection.OPERATION_SUCCESS) {
            ...
            if (!network_response.isEmpty()) {
                ...
                if (response.getG_response_trans_type().equalsIgnoreCase(TransType.INVOICE_EXPIRY_RESPONSE.name()) && response.getG_status() == 1) {
...
//                        request.setInvoiceLink(jsonObject.getString("invoiceLink"));
                ...
                } else if (response.getG_response_trans_type().equalsIgnoreCase(TransType.INVOICE_EXPIRY_RESPONSE.name()) && response.getG_status() != 1) {
