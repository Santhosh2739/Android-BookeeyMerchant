        ...
        payment_confirm_print_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (ret == 0) {
                    ...
                    //tvshow.setText("Fail");
                    Toast.makeText(getApplicationContext(), "Printer not available on device..!", Toast.LENGTH_SHORT).show();
                }
    ...
    private void transactionDetailsArabic(int result) {
        ...
        try {
            ...
            if(response_obj.getG_status_description().contains("success")){
                ...
                result = mPrinter.printString(getString(R.string.status)+": " + getString(R.string.transaction_success), 25, Printer.Align.LEFT, false, false);


            }else{
                result = mPrinter.printString(getString(R.string.status)+": "+ response_obj.getG_status_description(), 25, Printer.Align.LEFT, false, false);

            }


            result = mPrinter.printString(getString(R.string.date)+": " + TimeUtils.getDisplayableDateWithSeconds(response_obj.getG_servertime(), new Date(response_obj.getServerTime())), 25, Printer.Align.LEFT, false, false);
            result = mPrinter.printString(getString(R.string.transaction_id)+": "+ response_obj.getTransactionId(), 25, Printer.Align.LEFT, false, false);
            result = mPrinter.printString(getString(R.string.cust_mobile_no)+": "+ maskString(response_obj.getCustomerMobileNumber(), 0, 4, 'X'), 25, Printer.Align.LEFT, false, false);
            if (response_obj.getTotalPrice() != 0) {
                result = mPrinter.printString(getString(R.string.transaction_amount)+": " + PriceFormatter.format(response_obj.getTotalPrice(), 3, 3) + " KWD", 25, Printer.Align.LEFT, false, false);
            }
            /*if (response_obj.getDiscountPrice() != 0) {
                result = mPrinter.printString("Discount Amt:" + PriceFormatter.format(response_obj.getDiscountPrice(), 3, 3) + " KWD", 25, Printer.Align.LEFT, false, false);
            }*/
            result = mPrinter.printString(getString(R.string.total_paymnet)+": "+ PriceFormatter.format(response_obj.getTxnAmount(), 3, 3) + " KWD", 25, Printer.Align.LEFT, false, false);
            result = mPrinter.printString("------------------------------------------", 30, Printer.Align.CENTER, false, false);
//            result = mPrinter.printString("Thank you for using", 25, Printer.Align.CENTER, true, false);
